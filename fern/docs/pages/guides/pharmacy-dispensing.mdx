...
Prescription Entry → Drug Name Resolution → Safety Screening → Alert Triage → Dispensing Decision
       │                    │                      │                │               │
   Pharmacist          SafeRx API            All 7 domains    Pharmacist UI     Pharmacist
...
|---------------|-------------|--------------|
| Arrhythmia | `arrhythmia` | I48.x, I49.x |
| Asthma | `asthma` | J45.x |
| Benign prostatic hyperplasia | `bph` | N40.x |
| Coronary artery disease | `coronary` | I25.x |
| Depression | `depression` | F32.x, F33.x |
| Diabetes (Type 1 or 2) | `diabetes` | E10.x, E11.x |
| Epilepsy | `epilepsy` | G40.x |
| GI bleeding | `gi_bleeding` | K92.x |
| Glaucoma | `glaucoma` | H40.x |
| Heart failure | `heart_failure` | I50.x |
| Hypertension | `hypertension` | I10.x, I11.x |
| Hyperthyroidism | `hyperthyroidism` | E05.x |
| Hypothyroidism | `hypothyroidism` | E03.x |
| Parkinson's disease | `parkinsons` | G20.x |
...
def map_patient_to_profile(erp_patient):
    ...
    # Medical conditions (see full list: arrhythmia, asthma, bph, coronary,
    # depression, diabetes, epilepsy, gi_bleeding, glaucoma, heart_failure,
    # hypertension, hyperthyroidism, hypothyroidism, parkinsons)
    ...
    if "I48" in erp_patient.icd10_codes:
        conditions.append("arrhythmia")
    if "I25" in erp_patient.icd10_codes:
        conditions.append("coronary")
    if "G20" in erp_patient.icd10_codes:
