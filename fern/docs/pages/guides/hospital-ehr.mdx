...
class CPOESafetyScreener:
    ...
    def _map_patient_profile(self, patient: dict) -> dict:
        ...
        icd_map = {
            ...
            "I50": "heart_failure",
            "J45": "asthma",
            "I25": "coronary",          # Chronic ischaemic heart disease
            "I48": "arrhythmia",        # Atrial fibrillation / flutter
            "G20": "parkinsons",        # Parkinson's disease
            "H40": "glaucoma",
            "E05": "hyperthyroidism",
            "E03": "hypothyroidism",
            "N40": "bph",              # Benign prostatic hyperplasia
            "K92": "gi_bleeding",       # GI haemorrhage
            "F32": "depression",
        }
...
def fhir_patient_to_profile(patient: dict, conditions: list) -> dict:
    ...
    snomed_map = {
        ...
        "49436004": "arrhythmia",       # Atrial fibrillation
        "13644009": "hyperthyroidism",  # Hyperthyroidism
        "40930008": "hypothyroidism",   # Hypothyroidism
        "49049000": "parkinsons",       # Parkinson's disease
        "23986001": "glaucoma",         # Glaucoma
        "266474003": "depression",      # Major depression
        "53741008": "coronary",          # Coronary heart disease
        "74474003": "gi_bleeding",      # GI haemorrhage
        "110417007": "bph",             # Benign prostatic hyperplasia
    }
    # SNOMED codes that also imply population flags
    cardiac_codes = {"42343007", "49436004", "53741008"}  # heart_failure, arrhythmia, coronary
    renal_codes = {"709044004"}    # Chronic kidney disease
    hepatic_codes = {"235856003"}  # Liver cirrhosis

    for condition in conditions:
        code = condition.get("code", {}).get("coding", [{}])[0].get("code", "")
        if code in snomed_map:
            saferx_conditions.append(snomed_map[code])
        if code in cardiac_codes:
            populations.append("cardiac")
        if code in renal_codes:
            populations.append("renal")
        if code in hepatic_codes:
            populations.append("hepatic")

    return {
        "populations": list(set(populations)),
        "conditions": list(set(saferx_conditions)),
    }
```
